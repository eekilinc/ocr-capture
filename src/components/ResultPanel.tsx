...
export const ResultPanel = memo(({
  ...
  const renderContent = () => {
    ...
    return (
      <div className="panel-body-scroll" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', height: '100%', overflowY: 'auto' }}>
        {qrResult && (
            ...
            <div className="entity-chips-list">
              {detectedEntities.map((entity, idx) => (
                <div key={idx} className={`entity-chip entity-chip-${entity.type}`}>
                  ...
                  {entity.type === "iban" && (
                    <>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
                      <span className="entity-text" title={entity.value}>{entity.display}</span>
                      <button className="chip-action-btn" onClick={() => handleCopyEntity(entity)} title={t("btnCopy")}>
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                      </button>
    ...
    >
      <div className="panel-header">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.35rem" }}>
          {!isCollapsed && text && !loading && !error && (
            <button
          ...
          {engine && !isCollapsed && (
            <span style={{ fontSize: "0.7rem", color: "var(--text-tertiary)", opacity: 0.7, fontWeight: 600, padding: "0 4px" }}>
              {engine.split(" ")[0]}
            </span>
          )}
      ...
      <div className="panel-content">
        ...
        {text && !loading && !error && !showWordMap && !isCollapsed && (
          <div className="copy-btn-wrapper" style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
            <select className="monitor-select" style={{ padding: '0.4rem 0.5rem', fontSize: '0.75rem' }} value={copyFormat} onChange={(e) => setCopyFormat(e.target.value as CopyFormat)}>
            ...
            <button className="btn btn-primary" style={{ padding: '0.5rem 0.9rem', flex: 1, justifyContent: 'center' }} onClick={handleCopyClick}>
